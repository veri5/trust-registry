...
CONFIG_YML = tests/load-test.yml
REPORT_JSON = tests/test-run-report.json
...
tests-load:
	API_URL=$$(aws cloudformation describe-stacks --stack-name $(STACK_NAME) \
		--query 'Stacks[0].Outputs[?OutputKey==`ApiUrl`].OutputValue' \
		--output text) artillery run --config $(CONFIG_YML) --output $(REPORT_JSON) $(CONFIG_YML)

tests-load-report:
	artillery report $(REPORT_JSON)
